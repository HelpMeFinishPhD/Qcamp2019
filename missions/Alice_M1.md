...
You represent common crowd, and Bob represents big companies. In the end, the link will be used to send sensitive information (i.e. credit card details) from Alice to Bob. **The goal of this mission is to establish the communication link between Alice and Bob.**
...
Objective: Construct a circuit capable of sending IR signals to Bob (while at the same time Bob needs to construct another circuit capable of receiving IR signals). The objective is accomplished when Alice successfully sends (and Bob successfully receives) the blinking signal.
...
Point allocation scheme:
* [Max] points upon correct, efficient, and stable implementation of the circuit, or
...
1. Construct the circuit according to the diagram, noting the polarity and the pin assignment. *If the polarity or pin assignment is incorrect, you might burn the components.* If you are unsure about amything at all, feel free to ask the facilitator.
...
This pedagogical exercise wil dwell a little bit deeper into how information is actually sent. Most implementations are done in binary representation, which are in strings of 1 and 0's. Then, how can we write numbers, words, symbols, or even emojis in this binary string?

Objective: Alice writes the message "Hello Qcamp!" in binary representation, and sends it to Bob. You will need to use the programs in **BinaryComm Package** to perform this task, and you will also need to complete the **conversion table form**. To get the maximum number of points, both Alice and Bob must participate in this exercise.
...
1. For Alice, convert each character from ASCII to binary, and complete the conversion table form. You can use the ASCII table, but the program `conv_ascii.py` might also be  useful.
...
Objective: Complete the `Asymmetrical Cryptography` handout. No cheating or copying from Bob allowed [insert stern warning].
...
1. If everything works nicely, both Alice and Bob can run `chatting.py`. Bob can turn into listening mode by pressing an `Enter` button. The chatting should happens both ways.
...
There will be a few messages that you need to send to Bob within 10 minutes after the conclusion of Mission 1. They will come in a secret document from the `GameMaster`. You have to **ensure the security of the content**, and that you **seal** the document and **return** it to the `GameMaster` after the conclusion of the mission. Listen to the explicit instructions from the `GameMaster` on when to send each message.
...
* Some points will be forfeited if Alice communicates with Bob in any way besides the chatting software.
